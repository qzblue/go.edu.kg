...
class _ProxyManagerState extends ConsumerState<ProxyManager> {
  Future<void> _updateProxy(ProxyState proxyState) async {
    ...
    debugPrint('[APP] _updateProxy: isStart=$isStart, systemProxy=$systemProxy, port=$port');
    if (isStart && systemProxy) {
      debugPrint('[APP] _updateProxy: calling startProxy on port $port');
      proxy?.startProxy(port, proxyState.bassDomain);
    } else {
      debugPrint('[APP] _updateProxy: calling stopProxy');
      proxy?.stopProxy();
    }
  }
