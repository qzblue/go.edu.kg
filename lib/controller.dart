...
extension ProfilesControllerExt on AppController {
  ...
  Future<void> autoUpdateProfiles() async {
    ...
    final globalAutoUpdate = prefs.getBool('sub_auto_update') ?? true;
    if (!globalAutoUpdate) return;

    final intervalDays = prefs.getInt('sub_update_interval_days') ?? 3;
  ...
  Future<void> updateGroups() async {
    try {
      ...
      _ref.read(groupsProvider.notifier).value = await retry(
        task: () async {
          attempt++;
          commonPrint.log('updateGroups attempt $attempt');
          ...
          commonPrint.log('updateGroups attempt $attempt got ${groups.length} groups');
          return groups;
        },
        maxAttempts: 20,
      ...
      commonPrint.log('updateGroups done, groups count: ${_ref.read(groupsProvider).length}');
    } catch (e) {
...
extension SetupControllerExt on AppController {
  ...
  Future<void> updateStatus(bool isStart, {bool isInit = false}) async {
    commonPrint.log('updateStatus called: isStart=$isStart, isInit=$isInit');
    if (isStart) {
      if (!isInit) {
        final res = await tryStartCore(true);
        commonPrint.log('tryStartCore result: $res');
        if (res) {
          return;
        }
        if (!_ref.read(initProvider)) {
          commonPrint.log('updateStatus: initProvider is false, returning');
          return;
        }
        commonPrint.log('updateStatus: calling handleStart');
        await globalState.handleStart([updateRunTime, updateTraffic]);
        commonPrint.log('updateStatus: handleStart done, applying profile');
  ...
  Future<Map<String, dynamic>> getProfile({
    ...
    commonPrint.log('getConfig keys: ${configMap.keys.toList()}, proxies count: ${(configMap['proxies'] as List?)?.length}, proxy-groups count: ${(configMap['proxy-groups'] as List?)?.length}');
  ...
  Future<void> _setupConfig([VoidCallback? preloadInvoke]) async {
    commonPrint.log('setup ===> profileId: ${_ref.read(currentProfileIdProvider)}, profile: ${_ref.read(currentProfileProvider)?.label}');
    ...
    commonPrint.log('setupConfig writing config to: $configFilePath, yaml length: ${yamlString.length}');
    await File(configFilePath).safeWriteAsString(yamlString);
    // 验证写入后的文件确实包含 proxies
    final verifyContent = await File(configFilePath).readAsString();
    commonPrint.log('setupConfig verify file length: ${verifyContent.length}, contains proxies: ${verifyContent.contains('proxies:')}');
    final message = await coreController.setupConfig(
      setupState: setupState,
      params: setupParams,
      preloadInvoke: preloadInvoke,
    );
    commonPrint.log('setupConfig result message: "$message"');
...
extension CoreControllerExt on AppController {
  ...
  Future<void> _connectCore() async {
    ...
    // After core connects, schedule an extra updateGroups in case config
