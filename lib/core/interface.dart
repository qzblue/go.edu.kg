...
abstract class CoreHandlerInterface with CoreInterface {
  ...
  Future<ProxiesData> getProxies() async {
    ...
    // The core may return data in two formats:
    // 1. Wrapped: {"proxies": {...}, "all": [...]}
    // 2. Flat: {"GLOBAL": {...}, "DIRECT": {...}, ..., "code": 0, "Port": 0}
    // We need to handle both.
    if (data.containsKey('proxies') && data['proxies'] is Map) {
      data['proxies'] ??= <String, dynamic>{};
      data['all'] ??= <String>[];
      final result = ProxiesData.fromJson(data);
      commonPrint.log('getProxies (wrapped): proxies=${result.proxies.length}, all=${result.all.length}');
      return result;
    }

    // Flat format: reconstruct ProxiesData from the flat map
    final metaKeys = {'code', 'Port'};
    final proxies = <String, dynamic>{};
    final all = <String>[];
    final groupTypes = {
      'Selector', 'URLTest', 'Fallback', 'Relay', 'LoadBalance',
    };
    ...
    for (final entry in data.entries) {
      ...
      if (entry.value is Map<String, dynamic>) {
        ...
        final type = (entry.value as Map<String, dynamic>)['type'];
        if (type != null && groupTypes.contains(type.toString())) {
          all.add(entry.key);
        }
      }
    }

    commonPrint.log('getProxies (flat): proxies=${proxies.length}, all=${all.length}, allNames=$all');
