  ...
  Future<void> _attachSocket(Socket socket) async {
    ...
    socket
        ...
        .listen((data) async {
          ...
          if (trimmed.contains('"getProxies"') && trimmed.contains('"data"')) {
            commonPrint.log('RAW getProxies length: ${trimmed.length}');
            // 查找 proxies 和 all 关键字
            commonPrint.log('contains "proxies": ${trimmed.contains('"proxies"')}');
            commonPrint.log('contains "all": ${trimmed.contains('"all"')}');
            // 输出最后200个字符看看结尾
            commonPrint.log('RAW getProxies end: ${trimmed.substring(trimmed.length > 300 ? trimmed.length - 300 : 0)}');
          }
          final dataJson = await trimmed.commonToJSON<dynamic>();
