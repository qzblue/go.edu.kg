...
func (result ActionResult) Json() ([]byte, error) {
	...
	if result.Method == getProxiesMethod {
		if err != nil {
			fmt.Fprintf(os.Stderr, "[CORE] Json() marshal error for getProxies: %v\n", err)
		} else {
			fmt.Fprintf(os.Stderr, "[CORE] Json() for getProxies size: %d\n", len(data))
		}
	}
	return data, err
}
...
func (result ActionResult) success(data interface{}) {
	...
	result.Data = data
	if result.Method == getProxiesMethod {
		fmt.Fprintf(os.Stderr, "[CORE] success() Data type: %T\n", data)
	}
	result.send()
}
