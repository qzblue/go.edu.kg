...
func handleGetProxies() ProxiesData {
	...
	fmt.Fprintf(os.Stderr, "[CORE] handleGetProxies nameList len: %d\n", len(nameList))

	proxies := make(map[string]constant.Proxy)

	tp := tunnel.Proxies()
	fmt.Fprintf(os.Stderr, "[CORE] handleGetProxies tunnel.Proxies len: %d\n", len(tp))
	for name, proxy := range tp {
		proxies[name] = proxy
	}
	...
	for _, proxy := range proxies {
		typeCounts[proxy.Type().String()]++
	}
	for t, count := range typeCounts {
		fmt.Fprintf(os.Stderr, "[CORE] proxy type %s count: %d\n", t, count)
	}
	// 诊断日志：各selector组的all列表中各类型代理数量
	for _, groupName := range allNames {
		...
		if p.Type() == constant.Selector {
			inner, _ := p.MarshalJSON()
			innerMap := map[string]any{}
			json.Unmarshal(inner, &innerMap)
			if allList, ok2 := innerMap["all"].([]any); ok2 {
				groupTypeCounts := make(map[string]int)
				for _, item := range allList {
					if proxyName, ok3 := item.(string); ok3 {
						if px, ok4 := proxies[proxyName]; ok4 && px != nil {
							groupTypeCounts[px.Type().String()]++
						}
					}
				}
				fmt.Fprintf(os.Stderr, "[CORE] selector [%s] all: %d items, types: %v\n", groupName, len(allList), groupTypeCounts)
			}
		}
	}
	...
	// 测试序列化
	testData, testErr := json.Marshal(pd)
	if testErr != nil {
		fmt.Fprintf(os.Stderr, "[CORE] ProxiesData marshal ERROR: %v\n", testErr)
	} else {
		fmt.Fprintf(os.Stderr, "[CORE] ProxiesData marshal OK, size: %d\n", len(testData))
	}
	// 测试 ActionResult 序列化
	...
	arData, arErr := json.Marshal(testResult)
	if arErr != nil {
		fmt.Fprintf(os.Stderr, "[CORE] ActionResult marshal ERROR: %v\n", arErr)
	} else {
		hasNewline := b.Contains(arData, []byte("\n"))
		fmt.Fprintf(os.Stderr, "[CORE] ActionResult marshal OK, size: %d, has 'proxies': %v, has newline: %v\n", len(arData), b.Contains(arData, []byte(`"proxies"`)), hasNewline)
	}
	return pd
}
