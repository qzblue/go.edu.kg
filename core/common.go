...
func applyConfig(params *SetupParams) error {
	...
	fmt.Fprintf(os.Stderr, "[CORE] applyConfig loading: %s\n", configPath)
	currentConfig, err = executor.ParseWithPath(configPath)
	if err != nil {
		fmt.Fprintf(os.Stderr, "[CORE] ParseWithPath error: %v\n", err)
		currentConfig, _ = config.ParseRawConfig(config.DefaultRawConfig())
	} else {
		fmt.Fprintf(os.Stderr, "[CORE] ParseWithPath success, proxies: %d, providers: %d\n", len(currentConfig.Proxies), len(currentConfig.Providers))
	}
	hub.ApplyConfig(currentConfig)
	tp := tunnel.Proxies()
	fmt.Fprintf(os.Stderr, "[CORE] after ApplyConfig, tunnel.Proxies count: %d\n", len(tp))
